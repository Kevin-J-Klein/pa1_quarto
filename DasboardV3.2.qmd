...
card(
    ...
    layout_columns( # Use layout_columns for side-by-side inside card
        ...
        card(
            ...
            uiOutput("metadata_columns_dd")
        ),
        card(
            card_header("Selected Groups"),
            uiOutput("selected_group_values") # Display unique values of Selected Metadata
        )
    )
)
...
output$metadata_columns_dd <- renderUI({
    ps <- physeq_obj()
    validate(need(!is.null(ps), "Test")) #Ensure data is loaded before proceeding
    metadata_var <- names(sample_data(ps))
    categorical_variables <- metadata_var[sapply(sample_data(ps), function(x) is.character(x) || is.factor(x))] #Filter for categorical variables
    selected_var <- if (length(categorical_variables) > 0) categorical_variables[[1]] else metadata_var[[1]]
    #Create the selectInput dropdown
    selectInput("metadata_group", "Groups", choices = metadata_var, selected = selected_var)
})
