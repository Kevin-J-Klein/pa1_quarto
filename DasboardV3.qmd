...
observeEvent(input$export_button, {
    ...
    # Pfad zur Quarto-Vorlage
    template_file <- "report_template.qmd" # Ersetze dies durch den tatsächlichen Pfad
    report_basename <- paste0("dashboard_report_", format(Sys.time(), "%Y%m%d_%H%M%S"))
    temp_dir <- tempdir()

    # Erfasse die Parameter, die an die Quarto-Vorlage übergeben werden sollen
    params <- list(
        richness_plot = plotlyOutput("richness_plot"), # So funktioniert es nicht direkt
        rarefaction_plot_asv = plotlyOutput("rarefaction_plot"), # So funktioniert es nicht direkt
        rarefaction_plot_species = plotOutput("rarefaction_plot_spec"), # So funktioniert es nicht direkt
        metadata_table = output$metadata_columns # So funktioniert es nicht direkt
        # Füge hier weitere Parameter hinzu, die in der Vorlage verwendet werden sollen
    )

    if (export_format == "html") {
        output_format <- "html"
        report_filename <- paste0(report_basename, ".html")
    } else if (export_format == "pdf") {
        output_format <- "pdf"
        report_filename <- paste0(report_basename, ".pdf")
    } else {
    ...
    tryCatch({
        ...
        output$download_report <- downloadHandler(
            ...
            content = function(file) {
                file.copy(file.path(temp_dir, report_filename), file)
            }
        )
        session$sendCustomMessage("download", list(filename = report_filename))
...
# Hilfsausgabe für den Download
output$download_report <- downloadHandler(
    filename = "dummy.txt",
    content = function(file) {
        writeLines("Download vorbereitet...", file)
    }
)
